...
import { TOPICS, TOPIC_FILTERS, type PracticeTopic, type TopicFilter } from "../practice/topics";

export function PracticeCatalog() {
  const [selectedTopic, setSelectedTopic] = useState<PracticeTopic | null>(null);
  const [showLevelModal, setShowLevelModal] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedFilter, setSelectedFilter] = useState<TopicFilter>("Todos");
  ...
  const handleCloseModal = () => {
    ...
    // opcional: setSelectedTopic(null);
  };

  // ✅ por ahora NO navega, solo prueba UI
  const handleLevelSelect = (level: 1 | 2 | 3) => {
    // Luego acá conectamos con tu Practice viejo:
    // navigate(`/practice/${selectedTopic?.id}?level=${level}`)
    console.log("Selected:", selectedTopic?.id, "level:", level);
    setShowLevelModal(false);
  };
    ...
    <div className="space-y-6">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span
                  className={`text-xs px-3 py-1 rounded-full font-bold ${
                    isExam ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"
                  }`}
                >
      ...
      {filteredTopics.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 text-lg font-semibold">No se encontraron temas</p>
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 transform animate-in fade-in zoom-in duration-200">
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1">
                ...
                <h2 className="text-xl font-black text-gray-900 mb-1">{selectedTopic.title}</h2>
            ...
            <div className="space-y-3">
              ...
              {selectedTopic.levelsAvailable >= 1 && (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white p-4 rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-between group"
                >
              ...
              {selectedTopic.levelsAvailable >= 2 && (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 text-white p-4 rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-between group"
                >
                  <div className="text-left">
              ...
              {selectedTopic.levelsAvailable >= 3 && (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-red-500 to-pink-500 text-white p-4 rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-between group"
                >
                  <div className="text-left">
