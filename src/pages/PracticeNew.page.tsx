...
import { 
  Target, Search, X, CheckCircle, XCircle, ArrowRight, Clock,
  Type, Users, Zap, MessageCircle, Brain, Globe, 
  Briefcase, Coffee, BookOpen, TrendingUp, Link as LinkIcon, 
  Layers, MapPin, BarChart, Award, Sparkles, ChevronRight
} from "lucide-react";

// ... (mantendré todas las interfaces y datos de los tests aquí)
// Por ahora voy a crear la estructura visual
...
export function PracticeNew() {
  ...
  // Agrupar todos los temas con su metadata
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="text-center md:text-left flex-1">
          <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-gray-900">Práctica y Exámenes</h1>
            <Sparkles className="w-5 h-5 text-purple-600" />
          </div>
          <p className="text-sm sm:text-base text-gray-600">Domina el inglés con ejercicios y evaluaciones completas</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 sm:py-3 text-sm sm:text-base border-2 border-gray-200 rounded-xl focus:border-purple-500 focus:outline-none transition-colors"
          />
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {filters.map(filter => (
            <button
              ...
              className={`px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all duration-300 transform hover:scale-105 ${
        ...
        {filteredTopics.map(topic => {
          ...
          return (
            <div
              ...
              className={`group relative bg-white rounded-2xl p-4 sm:p-5 shadow-md hover:shadow-2xl transition-all duration-300 cursor-pointer transform hover:scale-105 hover:-translate-y-1 ${
            ...
            >
              ...
              <div className="relative mb-4">
                <div className={`absolute inset-0 bg-gradient-to-br ${topic.color} rounded-xl blur-xl opacity-40 group-hover:opacity-60 transition-opacity`}></div>
                <div className={`relative w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${topic.color} rounded-xl shadow-lg flex items-center justify-center group-hover:scale-110 transition-transform`}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-xs px-3 py-1 rounded-full font-bold ${
                  ...
                  {topic.levelsAvailable} {topic.levelsAvailable === 1 ? "Nivel" : "Niveles"}
                </span>
      ...
      {filteredTopics.length === 0 && (
        <div className="text-center py-12 sm:py-16">
          ...
          <p className="text-gray-500 text-sm sm:text-lg font-semibold">No se encontraron temas</p>
          <p className="text-gray-400 text-xs sm:text-sm">Intenta con otro término de búsqueda</p>
        </div>
      )}
      ...
      {showLevelModal && selectedTopic && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-5 sm:p-6 transform animate-in fade-in zoom-in duration-200">
            {/* Header */}
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1">
                <div className={`w-12 h-12 bg-gradient-to-br ${selectedTopic.color} rounded-xl shadow-lg flex items-center justify-center mb-3`}>
                  {(() => {
                    const Icon = selectedTopic.icon;
                    return <Icon className="w-6 h-6 text-white" strokeWidth={2.5} />;
                  })()}
                </div>
                <h2 className="text-lg sm:text-xl font-black text-gray-900 mb-1">{selectedTopic.title}</h2>
                <p className="text-xs sm:text-sm text-gray-600">{selectedTopic.description}</p>
              </div>
              <button
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-bold text-gray-700 mb-3">Selecciona un nivel:</h3>
              ...
              {selectedTopic.levelsAvailable >= 1 && (
                <button className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white p-4 rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-between group">
                  <div className="text-left">
                    <div className="text-xs sm:text-sm opacity-90">Nivel 1</div>
                    <div className="text-base sm:text-lg">Principiante</div>
                  </div>
              ...
              {selectedTopic.levelsAvailable >= 2 && (
                <button className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 text-white p-4 rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-between group">
                  <div className="text-left">
                    <div className="text-xs sm:text-sm opacity-90">Nivel 2</div>
                    <div className="text-base sm:text-lg">Intermedio</div>
                  </div>
                  <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </button>
              )}
              ...
              {selectedTopic.levelsAvailable >= 3 && (
                <button className="w-full bg-gradient-to-r from-red-500 to-pink-500 text-white p-4 rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-between group">
                  <div className="text-left">
                    <div className="text-xs sm:text-sm opacity-90">Nivel 3</div>
                    <div className="text-base sm:text-lg">Avanzado</div>
                  </div>
                  <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </button>
              )}
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <div className="flex items-center justify-between text-xs sm:text-sm">
                ...
                <span className="font-bold text-gray-900">{selectedTopic.totalQuestions}</span>
              </div>
